# 👋 Hi, I'm Purushottam Kumar

🎓 MCA Final Year Student (2024–2026)
📊 Aspiring Data Analyst | Data Science Enthusiast | AI/ML Learner
📍 India

---

# 🚀 About Me

I am a passionate MCA student with strong skills in Data Analytics, SQL, Power BI, Python, and Machine Learning fundamentals.
I enjoy analyzing real-world datasets, building interactive dashboards, and extracting meaningful business insights through data-driven solutions.

🔍 Interested in Data Analytics & Machine Learning
📊 Skilled in Data Cleaning, EDA & Data Visualization
📈 Passionate about Business Insights & Predictive Analytics
💡 Love solving real-world problems using data

---

# 🛠️ Tech Stack

## 💻 Languages

* Python
* SQL
* Java

## 📊 Data Analytics & AI/ML

* Pandas
* NumPy
* Matplotlib
* Seaborn
* Scikit-learn
* Data Cleaning & EDA
* Predictive Analytics

## 📈 Visualization & BI

* Power BI
* Looker Studio
* Excel

## ⚙️ Tools & Platforms

* Git & GitHub
* VS Code
* Google Colab
* MySQL
* Flask

---

# 📂 Projects

## 📊 E-commerce Supply Chain & Customer Behavior Analysis

* Performed EDA and business analysis on Amazon sales dataset
* Built SQL-based operational and sales analysis
* Identified customer behavior, regional demand, and cancellation trends
* Developed business insights for supply chain optimization

## ❤️ HeartCare AI

* AI-based heart disease prediction system using Machine Learning
* Developed Flask-based web application for prediction
* Implemented preprocessing and classification models
* Future scope: ECG/X-ray integration

## 📈 Sales Analytics Dashboard

* Built interactive Power BI dashboard for sales analysis
* Analyzed revenue trends, customer behavior, and product performance
* Performed data cleaning and visualization using Python and SQL

---

# 📈 Currently Learning

* Advanced Machine Learning
* Data Visualization & Business Intelligence
* Backend Development with Flask APIs
* Data Structures & Algorithms (Java)

---

# 📫 Connect With Me

📧 Email: [bgspurushottamjha29@gmail.com](mailto:bgspurushottamjha29@gmail.com)
💼 LinkedIn: linkedin.com/in/purushottam-kumar29
💻 GitHub: github.com/Purushottambgs

---

# ⚡ Fun Fact

I believe data-driven decisions can create real-world impact. 🚀
